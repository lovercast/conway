...
typedef struct {
    ...
    uint16_t dictPos; // currrent position in dictionary, we need to store
                      // 0-4096 -- so there are at least 13 bits needed here
} LZWGenState;
...
static void newNode(uint16_t *pTree, const uint16_t LZWIndex,
                    const uint16_t initDictLen)
{
    uint16_t *pNode;
...
/* add new child node */
static void add_child(uint16_t *pTree, const uint16_t parentIndex,
                      const uint16_t LZWIndex, const uint16_t initDictLen,
                      const uint8_t index)
{
...
/* reset the dictionary of known LZW codes -- will reset the current code length
 * as well */
static void resetDict(LZWGenState *pContext, const uint16_t initDictLen)
{
    ...
    pContext->dictPos
        = initDictLen + 2; // reset current position in dictionary (number of
                           // colors + 2 for start and end code)
    ...
    for (i = 0; i < initDictLen; ++i) {
        memset(&(pContext->pTree[i * initDictLen]), 0,
               initDictLen * sizeof(uint16_t)); // reset all egdes of root nodes
                                                // of the LZW dictionary tree
    }
}

/* find next LZW code representing the longest pixel sequence that is still in
 * the dictionary*/
static uint32_t lzw_crawl_tree(LZWGenState *pContext, uint32_t strPos,
                               uint16_t parentIndex, const uint16_t initDictLen)
{
    uint16_t nextPixel;

    while (strPos < (pContext->numPixel - 1)) {
        if ((pContext->pTree[parentIndex * initDictLen
                             + pContext->pImageData[strPos + 1]])
            != 0) { // if pixel sequence is still in LZW-dictionary
            nextPixel
                = pContext->pImageData[strPos + 1]; // get color index of next
                                                    // pixel in image data
            parentIndex
                = pContext->pTree[parentIndex * initDictLen
                                  + nextPixel]; // the color index determines
                                                // branch in the dictionary tree
            ++strPos;
        } else { // if pixel sequence is not yet in the dictionary
            pContext->pLZWData[pContext->LZWPos]
                = parentIndex;    // write last LZW code in LZW data
            ++(pContext->LZWPos); // increment position in LZW data
            if (pContext->dictPos
                < MAX_DICT_LEN) { // if LZW-dictionary is not full yet
                add_child(pContext->pTree, parentIndex, pContext->dictPos,
                          initDictLen,
                          pContext->pImageData[strPos + 1]); // add new LZW code
                                                             // to dictionary
                ++(pContext->dictPos); // increase current position in the
                                       // dictionary
            } else {
                // the dictionary reached its maximum code => reset it (not
                // required by GIF-standard but mostly done like this)
    ...
    pContext->pLZWData[pContext->LZWPos]
        = parentIndex; // if the end of the image is reached, write last LZW
                       // code
    ++(pContext->LZWPos);
    return strPos + 1;
}
...
static uint32_t lzw_generate(CGIF_Frame *pFrame, LZWGenState *pContext)
{
    ...
    resetDict(
        pContext,
        pFrame->initDictLen); // reset dictionary and issue clear-code at first
    while (strPos
           < pContext
                 ->numPixel) { // while there are still image data to be encoded
        parentIndex = pContext->pImageData[strPos]; // start at root node
        strPos      = lzw_crawl_tree(
                 pContext, strPos, (uint16_t)parentIndex,
                 pFrame
                     ->initDictLen); // get longest sequence that is still in
                                // dictionary, return new position in image data
    }
    pContext->pLZWData[pContext->LZWPos]
        = pFrame->initDictLen + 1; // termination code
    return pContext->LZWPos + 1;   // return number of elements of LZW data
}

/* pack the LZW data into a byte sequence*/
static uint32_t create_byte_list(CGIF_Frame *pFrame, uint8_t *byteList,
                                 uint32_t lzwPos, uint16_t *lzwStr)
{
    uint32_t i;
    uint32_t dictPos; // counting new LZW codes
    uint16_t n
        = 2 * pFrame->initDictLen; // if n - pFrame->initDictLen == dictPos, the
                                   // LZW code size is incremented by 1 bit
    uint32_t bytePos  = 0;         // position of current byte
    uint8_t bitOffset = 0;         // number of bits used in the last byte
    uint8_t lzwCodeLen
        = pFrame->initCodeLen; // dynamically increasing length of the LZW codes
    int correctLater = 0; // 1: one empty byte too much if end is reached after
                          // current code, 0 otherwise

    byteList[0] = 0; // except from the 1st byte all other bytes should be
                     // initialized stepwise (below)
    // the very first symbol might be the clear-code. However, this is not
    // mandatory. Quote: "Encoders should output a Clear code as the first code
    // of each image data stream." We keep the option to NOT output the clear
    // code as the first symbol in this function.
    dictPos = 1;
    for (i = 0; i < lzwPos; ++i) { // loop over all LZW codes
        if ((lzwCodeLen < MAX_CODE_LEN)
            && ((uint32_t)(n - (pFrame->initDictLen))
                == dictPos)) { // larger code is used for the 1st time at i =
                               // 256 ...+ 512 ...+ 1024 -> 256, 768, 1792
            ++lzwCodeLen; // increment the length of the LZW codes (bit units)
            n *= 2;       // set threshold for next increment of LZW code size
        }
        correctLater
            = 0; // 1 indicates that one empty byte is too much at the end
        byteList[bytePos] |= ((
            uint8_t)(lzwStr[i] << bitOffset)); // add 1st bits of the new LZW
                                               // code to the byte containing
                                               // part of the previous code
        if (lzwCodeLen + bitOffset
            >= 8) { // if the current byte is not enough of the LZW code
            if (lzwCodeLen + bitOffset
                == 8) { // if just this byte is filled exactly
                byteList[++bytePos]
                    = 0; // byte is full -- go to next byte and initialize as 0
                correctLater = 1; // use if one 0byte to much at the end
            } else if (lzwCodeLen + bitOffset
                       < 16) { // if the next byte is not completely filled
                byteList[++bytePos] = (uint8_t)(lzwStr[i] >> (8 - bitOffset));
            } else if (lzwCodeLen + bitOffset
                       == 16) { // if the next byte is exactly filled by LZW
                                // code
                byteList[++bytePos] = (uint8_t)(lzwStr[i] >> (8 - bitOffset));
                byteList[++bytePos]
                    = 0; // byte is full -- go to next byte and initialize as 0
                ...
                byteList[++bytePos]
                    = (uint8_t)(lzwStr[i]
                                >> (8 - bitOffset)); // write part of LZW code
                                                     // to next byte
                byteList[++bytePos]
                    = (uint8_t)(lzwStr[i]
                                >> (16 - bitOffset)); // write part of LZW code
                                                      // to byte after next byte
            }
        }
        bitOffset = (lzwCodeLen + bitOffset)
                  % 8; // how many bits of the last byte are used?
        ++dictPos;     // increment count of LZW codes
        if (lzwStr[i]
            == pFrame->initDictLen) { // if a clear code appears in the LZW data
            lzwCodeLen = pFrame->initCodeLen; // reset length of LZW codes
            n = 2 * pFrame->initDictLen; // reset threshold for next increment
                                         // of LZW code length
            dictPos = 1;                 // reset (see comment below)
            // take first code already into account to increment lzwCodeLen
            // exactly when the code length cannot represent the current maximum
            // symbol. Note: This is usually done implicitly, as the very first
            // symbol is a clear-code itself.
        }
    }
    // comment: the last byte can be zero in the following case only:
    // terminate code has been written (initial dict length + 1), but current
    // code size is larger so padding zero bits were added and extend into the
    // next byte(s).
    if (correctLater) { // if an unneccessaray empty 0-byte was initialized at
                        // the end
...
static uint32_t create_byte_list_block(uint8_t *byteList,
                                       uint8_t *byteListBlock,
                                       const uint32_t numBytes)
{
    uint32_t i;
    uint32_t numBlock
        = numBytes / BLOCK_SIZE; // number of byte blocks with length BLOCK_SIZE
    uint8_t numRest
        = numBytes
        % BLOCK_SIZE; // number of bytes in last block (if not completely full)

    for (i = 0; i < numBlock; ++i) { // loop over all blocks
        byteListBlock[i * (BLOCK_SIZE + 1)]
            = BLOCK_SIZE; // number of bytes in the following block
        memcpy(byteListBlock + 1 + i * (BLOCK_SIZE + 1),
               byteList + i * BLOCK_SIZE,
               BLOCK_SIZE); // copy block from byteList to byteListBlock
    }
    if (numRest > 0) {
        byteListBlock[numBlock * (BLOCK_SIZE + 1)]
            = numRest; // number of bytes in the following block
        memcpy(byteListBlock + 1 + numBlock * (BLOCK_SIZE + 1),
               byteList + numBlock * BLOCK_SIZE,
               numRest); // copy block from byteList to byteListBlock
        byteListBlock[1 + numBlock * (BLOCK_SIZE + 1) + numRest]
            = 0; // set 0 at end of frame
        return 1 + numBlock * (BLOCK_SIZE + 1)
             + numRest; // index of last entry in byteListBlock
    }
    // all LZW blocks in the frame have the same block size (BLOCK_SIZE), so
    // there are no remaining bytes to be writen.
...
static uint8_t *LZW_GenerateStream(CGIF_Frame *pFrame, const uint32_t numPixel,
                                   const uint8_t *pImageData)
{
    ...
    pContext->pTree
        = malloc(sizeof(uint16_t) * pFrame->initDictLen
                 * MAX_DICT_LEN); // TBD check return value of malloc
    pContext->numPixel   = numPixel;
    pContext->pImageData = pImageData;
    pContext->pLZWData   = malloc(
          sizeof(uint16_t) * (numPixel + 2)); // TBD check return value of malloc
    pContext->LZWPos = 0;
    ...
    uint8_t
        *byteListBlock; // lzw-data packed in byte-list with 255-block structure
    uint64_t MaxByteListLen
        = MAX_CODE_LEN * lzwPos / 8ul + 2ul + 1ul; // conservative upper bound
    uint64_t MaxByteListBlockLen
        = MAX_CODE_LEN * lzwPos * (BLOCK_SIZE + 1ul) / 8ul / BLOCK_SIZE + 2ul
        + 1ul + 1ul;                   // conservative upper bound
    byteList = malloc(MaxByteListLen); // TBD check return value of malloc
    byteListBlock
        = malloc(MaxByteListBlockLen); // TBD check return value of malloc
...
static void initMainHeader(CGIF *pGIF)
{
    ...
    pGIF->aHeader[HEADER_OFFSET_PACKED_FIELD]
        = (x << 7); // M = 1 (see GIF specs): Global color map is present
    if (x) {
        // calculate needed size of global color table
        pow2GlobalPalette
            = calcNextPower2Ex(pGIF->config.numGlobalPaletteEntries);
        pow2GlobalPalette = (pow2GlobalPalette < 1)
                              ? 1
                              : pow2GlobalPalette; // minimum size is 2^1
        pGIF->aHeader[HEADER_OFFSET_PACKED_FIELD]
            |= ((pow2GlobalPalette - 1)
                << 0); // set size of global color table (0 - 7 in header + 1)
    }
    pGIF->aHeader[HEADER_OFFSET_PACKED_FIELD]
        |= (0uL << 4); // set color resolution (outdated - always zero)
}
...
static void initGlobalColorTable(CGIF *pGIF)
{
    ...
    memcpy(pGIF->aGlobalColorTable, pGlobalPalette,
...
static void initAppExtBlock(CGIF *pGIF)
{
    ...
    NETSCAPE_LOOPS(pGIF->aAppExt)
        = hU16toLE(pGIF->config.numLoops); // number of repetitions (animation)
}
...
CGIF *cgif_newgif(CGIF_Config *pConfig)
{
    ...
    // GIF should be animated? => init corresponding app extension header
    // (NETSCAPE2.0)
...
/* compare given pixel indices using the correct local or global color table;
 * returns 0 if the two pixels are RGB equal */
static int cmpPixel(CGIF *pGIF, CGIF_Frame *pCur, CGIF_Frame *pBef,
                    const uint8_t iCur, const uint8_t iBef)
{
    uint8_t *pBefCT; // color table to use for pBef
    uint8_t *pCurCT; // color table to use for pCur

    // TBD add safety checks
    pBefCT = (pBef->config.attrFlags & CGIF_FRAME_ATTR_USE_LOCAL_TABLE)
               ? pBef->aLocalColorTable
               : pGIF->aGlobalColorTable; // local or global table used?
    pCurCT = (pCur->config.attrFlags & CGIF_FRAME_ATTR_USE_LOCAL_TABLE)
               ? pCur->aLocalColorTable
               : pGIF->aGlobalColorTable; // local or global table used?
    return memcmp(pBefCT + iBef * 3, pCurCT + iCur * 3, 3);
}

/* optimize GIF file size by only redrawing the rectangular area that differs
 * from previous frame */
static uint8_t *doWidthHeightOptim(CGIF *pGIF, CGIF_Frame *pFrame,
                                   uint8_t const *pCurImageData,
...
FoundHeight:
    ...
    i = newTop;
    x = 0;
    while (cmpPixel(pGIF, pFrame, pFrame->pBef, pCurImageData[i * width + x],
                    pBefImageData[i * width + x])
           == 0) {
        ++i;
        if (i > (newTop + newHeight - 1)) {
            ++x; //(x==width cannot happen as goto Done is triggered in the only
    ...
    while (cmpPixel(pGIF, pFrame, pFrame->pBef, pCurImageData[i * width + x],
        ...
        ++i;
        if (i > (newTop + newHeight - 1)) {
            --x; //(x<newLeft cannot happen as goto Done is triggered in the
                 //only possible case before)
            i = newTop;
        }
    }
...
Done:
    ...
    pNewImageData
        = malloc(newWidth * newHeight); // TBD check return value of malloc
    for (i = 0; i < newHeight; ++i) {
        memcpy(pNewImageData + i * newWidth,
               pCurImageData + (i + newTop) * width + newLeft, newWidth);
    }
...
int cgif_addframe(CGIF *pGIF, CGIF_FrameConfig *pConfig)
{
    ...
    useLocalTable
        = (pFrame->config.attrFlags & CGIF_FRAME_ATTR_USE_LOCAL_TABLE) ? 1 : 0;
    hasTransparency
        = (pGIF->config.attrFlags & CGIF_ATTR_HAS_TRANSPARENCY) ? 1 : 0;
    // deactivate impossible size optimizations
    //  => in case user-defined transparency is used
    // CGIF_FRAME_GEN_USE_TRANSPARENCY and CGIF_FRAME_GEN_USE_DIFF_WINDOW are
    // not possible
    if (isFirstFrame || hasTransparency) {
        pFrame->config.genFlags &= ~(CGIF_FRAME_GEN_USE_TRANSPARENCY
                                     | CGIF_FRAME_GEN_USE_DIFF_WINDOW);
    }
    // switch off transparency optimization if color table is full (no free spot
    // for the transparent index), TBD: count used colors, adapt table
    ...
    if (useLocalTable) {
        pFrame->initCodeLen
            = calcInitCodeLen(pFrame->config.numLocalPaletteEntries);
    } else {
        pFrame->initCodeLen
            = calcInitCodeLen(pGIF->config.numGlobalPaletteEntries);
    }
    ...
    if (useLocalTable) {
        ...
        pow2LocalPalette
            = calcNextPower2Ex(pFrame->config.numLocalPaletteEntries);
        pow2LocalPalette                         = (pow2LocalPalette < 1)
                                                     ? 1
                                                     : pow2LocalPalette; // minimum size is 2^1
        IMAGE_PACKED_FIELD(pFrame->aImageHeader) = (1 << 7);
        IMAGE_PACKED_FIELD(pFrame->aImageHeader)
            |= ((pow2LocalPalette - 1)
                << 0); // set size of local color table (0 - 7 in header + 1)
    }

    // check if we need to increase the initial code length in order to allow
    // the transparency optim. TBD revert this in case no transparency is
    // possible
    if (pFrame->config.genFlags & CGIF_FRAME_GEN_USE_TRANSPARENCY) {
        if (pFrame->initDictLen
            == (useLocalTable ? pFrame->config.numLocalPaletteEntries
                              : pGIF->config.numGlobalPaletteEntries)) {
            ++(pFrame->initCodeLen);
            pFrame->initDictLen = 1uL << (pFrame->initCodeLen - 1);
        }
    ...
    // copy given raw image data into the new CGIF_FrameConfig, as we might need
    // it in a later stage.
    pFrame->config.pImageData
        = malloc(imageWidth * imageHeight); // TBD check return value of malloc
    memcpy(pFrame->config.pImageData, pConfig->pImageData,
           imageWidth * imageHeight);

    // purge overlap of current frame and frame before (wdith - height optim),
    // if required (CGIF_FRAME_GEN_USE_DIFF_WINDOW set)
    if (pFrame->config.genFlags & CGIF_FRAME_GEN_USE_DIFF_WINDOW) {
        pTmpImageData = doWidthHeightOptim(pGIF, pFrame, pConfig->pImageData,
                                           pFrame->pBef->config.pImageData,
                                           imageWidth, imageHeight);
    } else {
    ...
    // mark matching areas of the previous frame as transparent, if required
    // (CGIF_FRAME_GEN_USE_TRANSPARENCY set)
    if (pFrame->config.genFlags & CGIF_FRAME_GEN_USE_TRANSPARENCY) {
        if (pTmpImageData == NULL) {
            pTmpImageData = malloc(
                imageWidth * imageHeight); // TBD check return value of malloc
            memcpy(pTmpImageData, pConfig->pImageData,
                   imageWidth * imageHeight);
        }
        pBefImageData = pFrame->pBef->config.pImageData;
        for (i = 0; i < pFrame->height; ++i) {
            for (x = 0; x < pFrame->width; ++x) {
                if (cmpPixel(pGIF, pFrame, pFrame->pBef,
                             pTmpImageData[i * pFrame->width + x],
                             pBefImageData[((pFrame->top + i) * imageWidth)
                                           + (pFrame->left + x)])
                    == 0) {
    ...
    if ((pFrame->config.genFlags & CGIF_FRAME_GEN_USE_TRANSPARENCY)
        || (pFrame->config.genFlags & CGIF_FRAME_GEN_USE_DIFF_WINDOW)) {
        pFrame->pRasterData = LZW_GenerateStream(
            pFrame, pFrame->width * pFrame->height, pTmpImageData);
        free(pTmpImageData);
    } else {
        pFrame->pRasterData = LZW_GenerateStream(
            pFrame, imageWidth * imageHeight, pConfig->pImageData);
    }
    ...
    pFrame->pNext
        = malloc(sizeof(CGIF_Frame)); // TBD check return value of malloc
    ...
    if (pGIF->config.attrFlags & CGIF_ATTR_IS_ANIMATED) {
        ...
        if ((pFrame->config.genFlags & CGIF_FRAME_GEN_USE_TRANSPARENCY)
            || hasTransparency) {
            pFrame->aGraphicExt[3]
                |= 0x01; // set flag indicating that transparency is used
        }
